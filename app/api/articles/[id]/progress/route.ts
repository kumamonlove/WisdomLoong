...
export async function PATCH(
  ...
  const body = (await request.json()) as { page?: number; positionY?: number; positionX?: number };
  ...
  const result = await database.query(
    `INSERT INTO reading_progress (user_id, article_id, page_number, position_y, position_x)
     SELECT $1, articles.id, $3, $4, $5 FROM articles WHERE articles.id = $2
     ...
     DO UPDATE SET
       ...
       updated_at = NOW()
     RETURNING page_number, position_y, position_x`,
    [user.id, articleId, page, positionY, positionX],
  );
  if (result.rowCount === 0) {
    return NextResponse.json({ error: "文章不存在" }, { status: 404 });
  }
  return NextResponse.json({ ok: true, page, positionY, positionX });
}
